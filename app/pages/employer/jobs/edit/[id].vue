...
function assignJobForm(job: Awaited<ReturnType<typeof getJobDetail>>) {
  ...
  jobForm.value.benefit = job.benefit ? job.benefit.split(',').filter(Boolean) : []
  ...
  const matchedExp = experienceOptions.find(o => o.min === job.experience_min && o.max === job.experience_max)
  jobForm.value.experience = matchedExp?.value || 'unlimited'
  jobForm.value.keywords = Array.isArray(job.keywords) ? [...job.keywords] : []
...
<template>
  <div>
    <div class="mb-[16px] flex gap-[8px] items-center">
      ...
      <span class="text-[14px] text-[#222222] leading-none">编辑职位</span>
    </div>
