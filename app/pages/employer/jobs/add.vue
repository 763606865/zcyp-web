...
function buildPayload(status: number) {
  const keywords = jobForm.value.keywords ? jobForm.value.keywords.split(KEYWORD_SEP_RE).filter(Boolean).slice(0, 20) : null
  return {
...
async function handleSave() {
  ...
  try {
    ...
    errorMessage.value = error instanceof ApiRequestError ? error.message : '保存失败。'
  }
  finally {
      ...
      <div class="space-y-5">
        ...
        <div class="rounded-[20px] bg-white p-6 shadow-[0_8px_24px_rgba(148,92,0,0.06)] ring-1 ring-[#f1e4c6]">
          ...
          <div class="mt-5 space-y-4">
            ...
            <div class="text-[13px] text-[#8a6b34] space-y-2">
              <span>详细地址</span>
              <AmapLocationPicker v-model="jobForm.workplace" placeholder="点击在地图中选择位置" />
            </div>
        ...
        <div class="flex gap-3">
          ...
          <button type="button" class="h-[46px] border border-[#f4d2d2] rounded-[14px] bg-white px-5 text-[14px] text-[#a27a2b]" @click="router.push('/employer/jobs')">
            取消
          </button>
        </div>
      </div>
